...
{
    long long alpha = (n * (1LL) * (n - 1) * (1LL) * (n - 2)) / 6;
    return alpha;
}
int main()
{
    ...
    while (t--)
    {
        ...
        long long all_value = total(n);
        long long excluded_value = 0;
        map<long long, long long> mp1, mp2;
        map<pair<long long, long long>, long long> mp3;
        for (int i = 0; i < vp.size(); ++i)
        {
            if (mp1.find(vp[i].first) != mp1.end() && mp2.find(vp[i].second) != mp2.end())
            {
                excluded_value += mp1[vp[i].first]*(1LL)*(n-mp2[vp[i].second])+mp2[vp[i].second]*(1LL)*(n-mp1[vp[i].first]);
                debug(excluded_value);
                if (mp3.find({vp[i].first, vp[i].second}) != mp3.end())
                {
                    excluded_value -= mp3[{vp[i].first, vp[i].second}]*();
                }
            }
            else if (mp1.find(vp[i].first) != mp1.end())
            {
                excluded_value += mp1[vp[i].first];
                excluded_value = excluded_value * (1LL) * (n - 2);
            }
            else if (mp2.find(vp[i].second) != mp2.end())
            {
                excluded_value += mp2[vp[i].second];
                excluded_value = excluded_value * (1LL) * (n - 2);
            }
            mp1[vp[i].first]++;
            mp2[vp[i].second]++;
            mp3[{vp[i].first, vp[i].second}]++;
        }
        long long final_answer = all_value - excluded_value;
        cout << final_answer << "\n";
    }
}