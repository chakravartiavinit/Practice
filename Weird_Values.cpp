...
int main()
{
    ...
    while (t--)
    {
        ...
        for (auto x : mp)
        {
            long long number = x.first;
            vector<long long> vt = mp[number];
            if (vt.size() > number)
            {
                vt.push_back(n);
                for (int i = number; i < vt.size() - 1; i++)
                {
                    int left_pos = vt[i + 1] - vt[i];
                    int right_pos = vt[i - number + 1] - vt[i - number];
                    answer += number * (1LL) * left_pos * (1LL) * right_pos;
                }
            }
        }
        cout << answer << "\n";
