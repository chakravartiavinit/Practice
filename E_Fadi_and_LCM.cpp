    ...
    else
    {
        ...
        for (int i = 1; i <= sqrt(alpha); ++i)
        {
            if (alpha % i == 0)
            {
                ...
                {
                    factors.push_back(i);
                    factors.push_back(alpha / i);
                }
            }
        }
        debug(factors);
        long long m = LCM(factors[0], factors[1]);
        debug(m);
        int mt = factors.size();
        debug(mt);
        long long first, second, MINI = 1e14;
        sort(factors.begin(), factors.end());
        int p1 = 0, p2 = factors.size() - 1;
        while (p1 < p2)
        {
            if (LCM(factors[p1], factors[p2]) == alpha && max(factors[p1], factors[p2]) < MINI)
            {
                first = factors[p1];
                second = factors[p2];
            }
