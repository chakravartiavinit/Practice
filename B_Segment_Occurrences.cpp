...
int main()
{
    ...
    for (int i = 0; i < n - m + 1; ++i)
    {
        int temp = 0;
        bool flag = true;
        for (int j = i; temp < m; ++j, ++temp)
        {
            if (alpha[j] != alpha[temp])
            {
                flag = false;
                break;
            }
    ...
    for (int i = 1; i < n; ++i)
    {
        preffix[i] = preffix[i] + preffix[i - 1];
    }
    debug(preffix);
    while (q--)
    {
        ...
        else
        {
            if (left == 1)
            {
                cout << preffix[right - 1] << "\n";
            }
            else
            {
                cout << preffix[right - 1] - preffix[left - 2] << "\n";
            }
        }
    }
}