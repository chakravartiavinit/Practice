...
/*###############################################################################################################################################*/
int check(int start,vector<int> &vt,int n)
{
    queue<pair<int,pair<int,int>>> qp;
    qp.push({start,{start,0}});
    while(qp.size()>0)
    {
        pair<int,pair<int,int>> alpha=qp.front();
        qp.pop();
        if(alpha.second.first>=0&&alpha.second.first<n)
        {
            if(vt[alpha.second.first]%2!=vt[alpha.first]%2)
            {
                return alpha.second.second;
            }
            else
            {
                if(alpha.second.first-vt[alpha.second.first]!=vt[alpha.first])
                {
                    qp.push({alpha.second.first,{alpha.second.first-vt[alpha.second.first],alpha.second.second+1}});
                }
                qp.push({alpha.second.first,{alpha.second.first+vt[alpha.second.first],alpha.second.second+1}});
            }
        }
