...
/*###############################################################################################################################################*/
bool check(long long source, long long Max, set<long long> &st, vector<long long> &ans, int count, int n)
{

    if (st.find(source) != st.end())
    {
        if (count != n - 1)
        {
    ...
    if (source % 3 == 0)
    {
        return (check(source / 3, Max, st, ans, count + 1, n) || check(2 * source, Max, st, ans, count + 1, n));
    }
    return check(2 * source, Max, st, ans, count + 1, n);
}
int main()
{
#ifndef ONLINE_JUDGE
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        bool c1 = check(vt[i], Max, temp, answer, count, n);
        if (c1 == true)
        {
            for (auto x : answer)
            {
