...
/*###############################################################################################################################################*/
long long answer(string left, string right, int i, int n, string &alpha)
{
    if (i == n)
    {
        long long l = 0, r = 0;
        ...
        {
            l += pow(10, counter) * (1LL) * (left[k] - '0');
            counter++;
        }
        counter = 0;
        for (int k = right.length() - 1; k >= 0; --k)
        {
            r += pow(10, counter) * (1LL) * (right[k] - '0');
            counter++;
        }

        return l * r;
    }
    return max(answer(left + alpha[i], right, i + 1, n, alpha), answer(left, right + alpha[i], i + 1, n, alpha));
}
int main()
{
#ifndef ONLINE_JUDGE
    ...
    string t, s;
    long long ans = answer(t, s, 0, n, alpha);
    cout << ans << "\n";
}