...
/*###############################################################################################################################################*/
void dfs(int source, map<int, vector<int>> &graph, vector<int> &topo, vector<bool> &visited)
{
    visited[source] = true;
    for (auto x : graph[source])
    {
        if (visited[x] == false)
        {
            dfs(x, graph, topo, visited);
        }
    }
    topo.push_back(source);
}
void dfs1(int source, map<int, vector<int>> &graph1, vector<bool> &visited1, vector<int> &a1)
{
    visited1[source] = true;
    a1.push_back(source);
    for (auto x : graph1[source])
    {
        if (visited1[x] == false)
        {
            dfs1(x, graph1, visited1, a1);
        }
    }
}
int main()
{
    ...
    cout.tie(NULL);
    int A = 6;
    vector<vector<int>> B = {{5, 3}, {3, 5}, {1, 2}, {4, 1}, {2, 4}, {3, 4}, {5, 2}, {3, 1}};
    map<int, vector<int>> graph, transpose_graph;
    for (int i = 0; i < B.size(); ++i)
    {
        graph[B[i][0]].push_back(B[i][1]);
        transpose_graph[B[i][1]].push_back(B[i][0]);
    }
    vector<int> topo;
    ...
    {
        if (visited1[topo[i]] == false)
        {
            vector<int> ans;
            dfs1(topo[i], transpose_graph, visited1, ans);
