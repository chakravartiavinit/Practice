...
int main()
{
    ...
    unsigned long long summation_all = 0;
    long long sum = 0;
    for (int i = 0; i < n; ++i)
    {
        cin >> vt[i];
        sum += vt[i];
        summation_all += ((vt[i]) * (1LL) * (vt[i] + 1)) / 2;
    }
    long long answer = 0;
    if (k >= sum)
    {
    ...
    else
    {
        ...
        for (int i = 0; i + 1 < n; ++i)
        {
            long long diff = abs(vt[i] - vt[i + 1]);
            debug(diff);
            debug(answer);
            if (k >= (counter * diff))
            {
                vt[i] -= diff;
                k -= counter * (1LL) * diff;
                answer += (diff * (1LL) * (2 * vt[i] - diff + 1) * (counter)) / 2;
                counter++;
            }
            else
            {
                long long delta = k / counter;
                answer += ((delta)*delta * (1LL) * (2 * vt[i] - delta + 1)) / 2;
                if (delta % n != 0)
                {
                    k = 0;
                    answer += vt[i] - delta - 1;
