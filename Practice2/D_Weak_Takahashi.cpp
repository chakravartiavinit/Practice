...
int x[4] = {1, 0};
int y[4] = {0, 1};
bool check(int i, int j, int n, int m, vector<vector<char>> &vt)
{
    if (i < 0 || j < 0 || i >= n || j >= m || vt[i][j] == '#')
    {
        return false;
    }
    return true;
}
long long dfs(int i, int j, int n, int m, vector<vector<char>> &vt)
{
    long long answer = 1;
    ...
    while (qp.size() > 0)
    {
        pair<int, int> alpha = qp.front();
        qp.pop();
        for (int k = 0; k < 2; ++k)
        {
            if (check(alpha.first + x[k], alpha.second + y[k], n, m, vt) == true)
            {
                vt[alpha.first + x[k]][alpha.second + y[k]] = '#';
                answer = max(answer, dist[alpha.first][alpha.second] + 1);
                dist[alpha.first + x[k]][alpha.second + y[k]] = dist[alpha.first][alpha.second] + 1;
                qp.push({alpha.first + x[k], alpha.second + y[k]});
            }
        }
    }
    return answer;
...
int main()
{
    ...
    long long answer = dfs(0, 0, n, m, vt);
    cout << answer << "\n";
}