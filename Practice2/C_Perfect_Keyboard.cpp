...
/*###############################################################################################################################################*/
void dfs(char alpha, vector<bool> &visited, map<char, vector<char>> &mp)
{
...
int main()
{
    ...
    while (t--)
    {
        ...
        {
            mp[alpha[i]].push_back(alpha[i + 1]);
            mp[alpha[i + 1]].push_back(alpha[i]);
        }
        debug(mp);
        int one = -1, more_than_two = -1;
        for (auto x : mp)
        {
            sort(x.second.begin(), x.second.end());
            mp[x.first].erase(unique(mp[x.first].begin(), mp[x.first].end()), mp[x.first].end());
        }
        for (auto x : mp)
        {
            if (x.second.size() >= 3)
            {
                more_than_two = x.first - 'a';
            }
