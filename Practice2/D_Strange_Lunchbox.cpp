...
int main()
{
    ...
    vector<pair<int, int>> vp = {{1, 4}, {2, 1}, {0, -2}, {4, 1}, {4, 3}, {-2, 3}, {-1, 1}};
    vector<bool> visited(vp.size() + 1, false);
    for (int i = 0; i < vp.size(); ++i)
    {
        for (int j = i + 1; j < vp.size(); ++j)
        {
            if (visited[j] == false)
            {
                int frst = vp[j].first - vp[i].first;
                int scnd = vp[j].second - vp[i].second;
                int alpha = __gcd(frst, scnd);
                debug(alpha);
                frst /= alpha;
                scnd /= alpha;
                mp[{min(frst, scnd), max(frst, scnd)}]++;
                visited[j] = true;
            }
        }
    }
