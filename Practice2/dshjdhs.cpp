...
int main()
{
    ...
    map<char, vector<char>> equal, not_equal;
    for (auto x : equations)
    {
        string temp = x;
        if (temp[1] == '!')
        {
            not_equal[temp[0]].push_back(temp[3]);
            not_equal[temp[3]].push_back(temp[0]);
        }
        else
        {
            equal[temp[0]].push_back(temp[3]);
            equal[temp[3]].push_back(temp[0]);
        }
    }
    debug(equal);
    debug(not_equal);
