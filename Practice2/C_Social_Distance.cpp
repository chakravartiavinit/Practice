...
/*###############################################################################################################################################*/
int Calc(int length, int k)
{
    return (length - 1) / (k + 1) + 1;
}
int main()
{
#ifndef ONLINE_JUDGE
    ...
    while (t--)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < first_one - k; ++i)
            {
                length1++;
            }
            for (int i = second_one + k; i < n; ++i)
            {
                length2++;
            }
            if (first_one - k >= 0)
            {
                final_answer += max(0, Calc(length1, k));
                // debug(final_answer);
            }
            if (second_one + k < n)
            {
                final_answer += max(0, Calc(length2, k));
                debug(final_answer);
            }
            bool end = false;
            // debug(final_answer);
            for (int i = first_one + 1; i < second_one; ++i)
            {
                if (alpha[i] == 'L')
                {
                    end = false;
