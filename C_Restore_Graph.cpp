...
int main()
{
    ...
    if (flag == false || count > k||start_node==-1)
    {
        cout << "-1\n";
    }
    else
    {
        ...
        map<int, vector<int>> distance_index;
        for (int i = 0; i < n; ++i)
        {
            vp.push_back({distance[i], i + 1});
            distance_index[distance[i]].push_back(i + 1);
        }
        sort(vp.begin(), vp.end());
        flag = true;
        for (int i = 1; i < vp.size(); ++i)
        {
            if (abs(vp[i].first - vp[i - 1].first) > 1)
            {
                flag = false;
                break;
            }
        ...
        else
        {
            for (int i = 0; i < n; ++i)
            {
                if (vp[i].first == 1)
                {
                    graph[start_node].push_back(vp[i].second);
                }
            }
            int alpha = 1;
            int Max = distance_index.rbegin()->first;
            while (alpha < Max)
            {
                int temp = 0, gamma = distance_index[alpha].size();
                while (temp < distance_index[alpha + 1].size())
                {
                    graph[distance_index[alpha][temp % gamma]].push_back(distance_index[alpha + 1][temp]);
                    temp++;
                }
                alpha++;
            }
