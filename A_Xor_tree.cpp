...
void dfs(int source, int parent, vector<int> &change, int &count)
{
    if (count % 2 == 0)
    {
        // count++;
        change[source - 1] = 1 - change[source - 1];
    }
...
int main()
{
    ...
    vector<int> final(n + 1), change(n + 1);
    for (int i = 0; i < n; ++i)
    {
        cin >> change[i];
    }
    ...
    while (flag)
    {
        ...
        {
            if (final[possible[i] - 1] != change[possible[i] - 1])
            {
                int count = 0;
                answer.push_back(possible[i]);
                flag = true;
                dfs(possible[i], possible[i], change, count);
            }
        }
        if (flag == false)
        {
