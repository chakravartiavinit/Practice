...
/*###############################################################################################################################################*/
long long answer(vector<int> &vt, int left, int z, long long sum, int k, int index, int count, int n)
{
    if (count == k + 1)
    {
        return sum;
    }
    long long alpha = INT_MIN;
    if (left == 0 && z > 0 && index >= 1)
    {
        alpha = max(alpha, answer(vt, left + 1, z - 1, sum + vt[index], k, index - 1, count + 1, n));
    }
    if (index >= 0 && index < n - 1)
    {
        alpha = max(alpha, answer(vt, left, z, sum + vt[index], k, index + 1, count + 1, n));
    }
    return alpha;
}
int main()
{
    ...
    while (t--)
    {
        ...
        long long ans = answer(vt, 0, z, 0, k, 0, 0, n);
        cout << ans << "\n";
    }
}