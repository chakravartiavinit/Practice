...
int main()
{
    ...
    while (t--)
    {
        ...
        int odd = 0, even = 0;
        vector<pair<long long, long long>> even_ele, odd_ele;
        for (int i = 0; i < n; ++i)
        {
            cin >> vt[i];
            if (vt[i] % 2 == 0)
            {
                even++;
                even_ele.push_back({i + 1, vt[i]});
            }
            else
            {
                odd++;
                odd_ele.push_back({i + 1, vt[i]});
            }
        }
        sort(even_ele.begin(), even_ele.end());
        sort(odd_ele.begin(), odd_ele.end());
        long long delta=((n*(1LL)*(n+1)))/2;
        if(odd==even)
        {
