...
int main()
{
    ...
    while (t--)
    {
        ...
        long long r_limit = n - 1;
        long long l_limit = max(0ll, n - 1000);
        long long ans = LLONG_MIN;
        for (long long i = l_limit; i <= r_limit; i++)
        {
            for (long long j = i + 1; j <= r_limit; j++)
            {
                ans = max(ans, (i + 1) * (j + 1) - k * (vt[i] | vt[j]));
            }
        }
        cout << ans << "\n";
    }
