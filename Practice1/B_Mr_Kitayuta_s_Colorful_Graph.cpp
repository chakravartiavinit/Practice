...
/*###############################################################################################################################################*/
void ans(int source, int dest, int color, vector<bool> &visited, int &count, map<int, vector<pair<int, int>>> &graph)
{
    if (source == dest)
    {
        count++;
    ...
    {
        if (visited[x.first] == false && x.second == color)
        {
            ans(x.first, dest, color, visited, count, graph);
        }
    }
}
int main()
{
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        graph[right].push_back({left, val});
        colors[left].insert(val);
        colors[right].insert(val);
    }
    int q;
    cin >> q;
    for (int i = 0; i < q; ++i)
    {
        ...
        cin >> start >> end;
        int count = 0;
        for (auto x : colors[start])
        {
            vector<bool> visited(101, false);
            ans(start, end, x, visited, count, graph);
        }
        cout << count << "\n";
    }
}