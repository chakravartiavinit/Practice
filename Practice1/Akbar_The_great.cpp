...
/*###############################################################################################################################################*/
bool CheckSoldier(map<long long, vector<long long>> mp, vector<bool> &visited, queue<pair<long long, pair<long long, long long>>> &q)
{
    while (q.size() > 0)
    {
        pair<long long, pair<long long, long long>> alpha;
        alpha = q.front();
        q.pop();
        pair<long long, long long> beta;
        beta = alpha.second;
        for (auto y : mp[beta.first])
        {
            if (y != alpha.first)
            {
                debug(visited);
                debug(alpha);
                if (beta.second > 0 && visited[y] == true)
                {
                    debug(y);
                    return false;
                }
                else
                {
                    ...
                    {
                        visited[y] = true;
                        q.push({beta.first, {y, beta.second - 1}});
                    }
                }
            }
        }
...
int main()
{
    ...
    while (t--)
    {
        ...
        debug(mp);
        bool ans = true;
        bool f = CheckSoldier(mp, visited, q);
        if (f == false)
        {
            ans = false;
        }
