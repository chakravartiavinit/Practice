...
int main()
{
    ...
    while (t--)
    {
        ...
        queue<pair<long long, long long>> qp;
        for (int i = 0; i < n; ++i)
        {
            cin >> vt[i];
            qp.push({vt[i], 1});
        ...
        while (qp.size() > 0)
        {
            pair<long long, long long> alpha = qp.front();
            qp.pop();
            if (alpha.first % x == 0 && flag == false)
            {
                qp.push({(alpha.first / x), (x * (1LL) * alpha.second)});
            }
            else
            {
                flag = true;
            }
            answer += alpha.first * (1LL) * alpha.second;
        }
        cout << answer << "\n";
    }
}