...
/*###############################################################################################################################################*/
bool compare(pair<pair<char, int>, int> p1, pair<pair<char, int>, int> p2)
{
    return p2.first.second < p1.first.second;
}
int main()
{
#ifndef ONLINE_JUDGE
    ...
    while (m--)
    {
        ...
        for (int i = 0; i < 26; ++i)
        {
            if (vt1[i] != 0)
            {
                answer = max(answer, vt[i][vt1[i] - 1]);
            }
        }
        cout << answer + 1 << "\n";
    }
