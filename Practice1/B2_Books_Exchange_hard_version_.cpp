...
/*###############################################################################################################################################*/
void dfs(int source, map<int, vector<int>> &graph, vector<int> &vt, vector<int> &change, vector<bool> &visited, int &count)
{
    visited[source] = true;
    for (auto x : graph[source])
    {
        if (visited[x] == false)
        {
            visited[x] = true;
            count++;
            dfs(x, graph, vt, change, visited, count);
        }
        else
        {
            if (change[source] == -1)
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<int> first(n + 10, -1);
        for (int i = 1; i <= n; ++i)
        {
            graph[vt[i]].push_back(vt[vt[i]]);
            if (i == vt[i])
            {
                first[i] = 1;
            }
        ...
        for (int i = 1; i <= n; ++i)
        {
            ...
            {
                int temp = count;
                dfs(i, graph, vt, first, visited, temp);
            }
        }
        for (int i = 1; i <= n; ++i)
        {
