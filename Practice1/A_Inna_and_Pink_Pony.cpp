...
/*###############################################################################################################################################*/
long long answer(int ans, int l, int r, int i, int j, int N, int M, int a, int b)
{
    if (i == l && j == r)
    {
        return 0;
    ...
    int alpha = abs(l - i);
    int beta = abs(r - j);
    if (alpha % a == 0 && beta % b == 0 && !(i + a > N && i - a < 1) && !(j + b > M && j - b < 1))
    {
        if ((alpha / a) % 2 == (beta / b) % 2)
        {
            return min(ans, max(alpha / a, beta / b));
        }
    }
    return INT_MAX;
}
int main()
{
    ...
    ans = min(ans, answer(ans, 1, 1, i, j, n, m, a, b));
    ans = min(ans, answer(ans, n, m, i, j, n, m, a, b));
    ans = min(ans, answer(ans, 1, m, i, j, n, m, a, b));
    ans = min(ans, answer(ans, n, 1, i, j, n, m, a, b));
