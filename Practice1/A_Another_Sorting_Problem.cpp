...
double Calc(map<string, vector<pair<string, double>>> &graph, string &start, string &parent, string &end, double final)
{
    if (start == end)
    {
        return final;
    ...
    for (auto x : graph[start])
    {
        if (x.first != parent)
        {
            Calc(graph, x.first, start, end, final * x.second);
        }
    }
}
void check(map<string, vector<pair<string, double>>> &graph, string &start, string &parent, string &end, bool &flag)
{
    if (start == end)
    {
        flag = true;
    ...
    for (auto x : graph[start])
    {
        if (x.first != parent)
        {
            check(graph, x.first, start, end, flag);
        }
    }
}
int main()
