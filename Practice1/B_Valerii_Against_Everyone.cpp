...
int main()
{
    ...
    while (t--)
    {
        ...
        long long Mod = 1000000021;
        for (int i = 0; i < n; ++i)
        {
            cin >> vt[i];
            mp[vt[i]]++;
            vt[i] = binpow(2, vt[i], Mod);
            vt[i] = (vt[i] % Mod);
        }
        ...
        for (int i = 1; i < n; ++i)
        {
            preffix[i] = (preffix[i - 1] % Mod + vt[i] % Mod) % Mod;
        }
        bool flag = false;
        debug(preffix);
        for (int i = 0; i + 1 < n; ++i)
        {
            long long first = preffix[i] % Mod;
            long long second = (preffix[n - 1] - preffix[i]) % Mod;
            if (first == second)
            {
                flag = true;
                break;
