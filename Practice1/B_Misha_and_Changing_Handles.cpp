...
/*###############################################################################################################################################*/
void dfs(string start, map<string, vector<string>> &graph, string &answer, map<string, bool> &visited)
{
    answer = start;
    visited[start] = true;
...
int main()
{
    ...
    int n;
    cin >> n;
    vector<pair<string, string>> vp, vp1;
    ...
    for (int i = 0; i < n; ++i)
    {
        if (visited[vp[i].first] == false)
        {
            string answer;
            dfs(vp[i].first, graph, answer, visited);
            vp1.push_back({vp[i].first, answer});
        }
    }
    cout << vp1.size() << "\n";
    for (auto x : vp1)
