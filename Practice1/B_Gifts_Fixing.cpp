...
int main()
{
    ...
    while (t--)
    {
        ...
        long long Min1 = 100000000000, Min2 = 100000000000;
        for (int i = 0; i < n; ++i)
        {
            cin >> alpha[i];
            Min1 = min(Min1, alpha[i]);
        }
        for (int i = 0; i < n; ++i)
        {
            cin >> beta[i];
            Min2 = min(Min2, beta[i]);
        }
        vector<pair<long long, long long>> vp;
        for (int i = 0; i < n; ++i)
        {
            vp.push_back({alpha[i], beta[i]});
        }
        long long answer = 0;
        for (int i = 0; i < n; ++i)
        {
            long long temp1 = vp[i].first - Min1;
            long long temp2 = vp[i].second - Min2;
