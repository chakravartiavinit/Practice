...
/*###############################################################################################################################################*/
long long Get(vector<long long> &parent,long long a)
{
    return parent[a]=(parent[a]==a?a:Get(parent,parent[a]));
}
void Union(vector<long long> &parent,long long a,long long b)
{
    a=Get(parent,a);
    b=Get(parent,b);
    parent[b]=a;
...
int main()
{
    ...
    for(int i=0;i<k;++i)
    {
        int query;
        cin>>query;
        vector<long long> alpha(query);
        long long Min=10000000000;
        int parent_of_this_group;
        for(int j=0;j<query;++j)
        {
            cin>>alpha[j];
            if(cost[alpha[j]-1]<Min)
            {
                Min=cost[alpha[j]-1];
                parent_of_this_group=alpha[j]-1;
                debug(Min);
                debug(parent_of_this_group);
            }
        }
        for(int j=0;j<query;++j)
        {
            Union(parent,parent_of_this_group,alpha[j]-1);
        }
    }
    long long cost_of_message=0;
    for(int i=0;i<m;++i)
    {
        string temp;
        cin>>temp;
        int x=mp[temp];
        cost_of_message+=cost[Get(parent,x)];
    }
    cout<<cost_of_message<<"\n";
