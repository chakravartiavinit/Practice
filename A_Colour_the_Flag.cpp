...
{
    if (i - 1 >= 0 && vt[i - 1][j] == vt[i][j])
    {
        return false;
    }
    if (i + 1 < n && vt[i + 1][j] == vt[i][j])
    {
        return false;
    }
    if (j - 1 >= 0 && vt[i][j] == vt[i][j - 1])
    {
        return false;
    }
    if (j + 1 < m && vt[i][j] == vt[i][j + 1])
    {
        return false;
    }
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<vector<char>> vt1(n, vector<char>(m));
        vector<vector<char>> vt2(n, vector<char>(m));
        for (int i = 0; i < n; ++i)
        {
            for (int j = 0; j < m; ++j)
            {
