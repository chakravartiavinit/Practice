...
/*###############################################################################################################################################*/
bool check(vector<int> vt, int index, int sum, int n)
{
    if (index == n - 1)
    {
        if ((sum + vt[index] == 0) || (sum - vt[index] == 0) || (sum + vt[index]) % 360 == 0 || (sum - vt[index]) % 360 == 0)
        {
            return true;
        }
        return false;
    }
    return check(vt, index + 1, sum + vt[index], n) || check(vt, index + 1, sum - vt[index], n);
}
int main()
{
#ifndef ONLINE_JUDGE
