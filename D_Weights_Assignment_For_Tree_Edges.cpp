...
/*###############################################################################################################################################*/
void dfs(int start, int parent, map<int, vector<int>> &graph, vector<int> &dist, map<pair<int, int>, int> &edges)
{
    dist[start] = dist[parent] + edges[{parent, start}];
    for (auto x : graph[start])
    {
        if (x != parent)
        {
            dfs(x, start, graph, dist, edges);
        }
    }
}
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 1; i <= n; ++i)
        {
            ...
            graph[vt[i]].push_back(i);
        }
        debug(graph);
        debug(root);
        vector<int> dist_priority(n + 1);
        for (int i = 1; i <= n; ++i)
        {
            cin >> dist_priority[i];
        }
        ...
        for (int i = 1; i <= n; ++i)
        {
            edges[{vt[dist_priority[i]], dist_priority[i]}] = temp;
            temp++;
        }
        debug(edges);
        vector<int> distance_from_root(n + 1, 0);
        dfs(root, root, graph, distance_from_root, edges);
        debug(distance_from_root);
    }
}