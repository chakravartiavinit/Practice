...
int main()
{
    ...
    int Diff = pq1.top().first - pq2.top().first;
    debug(Diff);
    for (int i = 0; i < k; ++i)
    {
        pair<int, int> p1, p2;
        ...
        p1.first--;
        p2.first++;
        int temp_diff = abs(p1.first - p2.first);
        if (temp_diff < Diff)
        {
            debug(p1.first);
            debug(p2.first);
            ...
            pq1.push({p1.first, p1.second});
            pq2.push({p2.first, p2.second});
            Diff = temp_diff;
        }
        else
        {
    ...
    cout << abs(pq1.top().first - pq2.top().first) << " " << vp.size() << "\n";
    for (auto x : vp)
    {
