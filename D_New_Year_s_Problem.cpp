...
int main()
{
    ...
    while (t--)
    {
        int n, m;
        cin >> n >> m;
        vector<vector<long long>> vt(n, vector<long long>(m));
        for (int i = 0; i < n; ++i)
        {
            for (int j = 0; j < m; ++j)
            {
        ...
        long long answer = 0;
        vector<vector<long long>> min_value(n, vector<long long>(m));
        vector<vector<long long>> next_greater(n, vector<long long>(m));
        for (int i = 0; i < n; ++i)
        {
            for (int j = 0; j < m; ++j)
            {
                if (j == m - 1)
                {
                    min_value[i][j] = vt[i][j];
                }
                else
                {
                    min_value[i][j] = min(vt[i][j], vt[i][j + 1]);
                }
            }
        }
        for (int i = 0; i < n; ++i)
        {
            long long Max = vt[i][m - 1];
            next_greater[i][m - 1] = Max;
            for (int j = m - 2; j >= 0; --j)
            {
                Max = max(Max, vt[i][j]);
                next_greater[i][j] = Max;
            }
        }
        debug(min_value);
        debug(next_greater);
