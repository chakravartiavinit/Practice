...
int x[4] = {0, 0, 1, -1};
int y[4] = {-1, 1, 0, 0};
bool check(int i, int j, int n, int m, vector<vector<int>> &vt)
{
    if (i < 0 || j < 0 || i >= n || j >= m || vt[i][j] == 0)
    {
        return false;
    }
    return true;
}
int dfs(int i, int j, int n, int m, vector<vector<int>> &vt, vector<vector<bool>> &visited)
{
    int answer = 0;
    ...
    while (qp.size() > 0)
    {
        ...
        for (int k = 0; k < 4; ++k)
        {
            ...
            {
                c++;
                if (visited[alpha.first + x[k]][alpha.second + y[k]] == false)
                {
                            visited[alpha.first][alpha.second] = true;
...
int islandPerimeter(vector<vector<int>> &grid)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
            ...
            {
                visited[i][j] = true;
                answer += dfs(i, j, n, m, grid, visited);
            }
        }
    }
    return answer;
