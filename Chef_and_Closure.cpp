...
int main()
{
    ...
    while (t--)
    {
        ...
        set<long long> st;
        set<long long> positive;
        set<long long> negative;
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (vt[i] > 0 && vt[i] != 1)
            {
                positive.insert(vt[i]);
                pos++;
            }
            else if (vt[i] < 0 && vt[i] != -1)
            {
                negative.insert(vt[i]);
                neg++;
            }
            mp[vt[i]]++;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    long long gamma = vt[0] * (1LL) * (vt[1]);
                    if (st.find(gamma) == st.end())
                    {
                        flag = false;
                    }
                    long long gamma1 = vt[0] * (1LL) * (vt[1]);
                    if (st.find(gamma) == st.end())
                    {
                        flag = false;
                    }
                    long long gamma2 = vt[n - 1] * (1LL) * (vt[n - 2]);
                    if (st.find(gamma2) == st.end())
                    {
                        flag = false;
                    }
                    long long gamma3 = vt[n - 1] * (1LL) * (vt[0]);
                    if (st.find(gamma3) == st.end())
                    {
                        flag = false;
                    }
                }
