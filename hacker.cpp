...
/*###############################################################################################################################################*/
void dfs(char fixed, char start, char parent, map<char, set<char>> &mp, map<char, set<char>> &num)
{
    num[fixed].insert(start);
    for (auto x : mp[start])
    {
        if (x != parent)
        {
            dfs(fixed, x, start, mp, num);
        }
        else
        {
            return;
...
string solve(string a, string b, string target)
{
    ...
    for (auto x : mp)
    {
        dfs(x.first, x.first, x.first, mp, num);
    }
    debug(num);
    string answer;
    for (auto x : target)
